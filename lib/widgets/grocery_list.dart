...
class _GroceryListState extends State<GroceryList> {
  ...
  void  _loadItems()async {
    final url=Uri.https(
  'flutter-prep-64e86-default-rtdb.firebaseio.com','shopping-list.json');
 ...
 for (final item in listData.entries){
  ...
  loadedItems.add(GroceryItem(
    id: item.key, name: item.value['name'], quantity: item.value['quantity'],
     category: category));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            onPressed: _addItem, icon: Icon (Icons.add),)
        ],
      ),
      body: content,
